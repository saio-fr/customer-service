...
# create rc config
chmod +x tasks/deploy/customer-controller.yml.sh && tasks/deploy/customer-controller.yml.sh
...
if [ $($KUBERNETES_CMD get rc | grep -c customer) -ne 1 ]; then
    echo "Create customer rc with replicas:" $REPLICAS_NUMBER
    $KUBERNETES_CMD create -f tasks/deploy/customer-controller.yml
else
    echo "Rolling update authorizer rc"
    $KUBERNETES_CMD rolling-update customer --update-period=10s --image=${EXTERNAL_REGISTRY_ENDPOINT}/customer:${CIRCLE_BRANCH}.${CIRCLE_SHA1}
fi
